...
class RecruitingScorecardService {
  ...
  async upsert(scorecard: RecruitingScorecardRecord): Promise<void> {
    const key = 'talentSonar:recruitingScorecards';
    const items = this.readLocal();
    const now = new Date().toISOString();
    const existing = items.find((item) => item.candidateId === scorecard.candidateId && item.jobId === scorecard.jobId && item.version === scorecard.version);
    const next = { ...scorecard, id: scorecard.id ?? existing?.id ?? Date.now(), createdAt: scorecard.createdAt ?? existing?.createdAt ?? now, updatedAt: now };
    localStorage.setItem(key, JSON.stringify([next, ...items.filter((item) => !(item.candidateId === next.candidateId && item.jobId === next.jobId && item.version === next.version))]));
  }

  async listForCandidate(candidateId: string, version = SCORECARD_VERSION, limit = 250): Promise<RecruitingScorecardRecord[]> {
    return this.readLocal().filter((item) => item.candidateId === candidateId && item.version === version).slice(0, limit);
  }

  private readLocal(): RecruitingScorecardRecord[] {
    try { const value = JSON.parse(localStorage.getItem('talentSonar:recruitingScorecards') || '[]'); return Array.isArray(value) ? value : []; }
    catch { return []; }
  }
}
