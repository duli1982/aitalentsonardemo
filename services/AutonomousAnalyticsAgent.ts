...
export interface PipelineSnapshot {
    ...
    talentPool: {
        localStoreCandidateCount: number | null;
    };
}
...
class AutonomousAnalyticsAgent {
    ...
    private async fetchLocalWorkspaceCandidateCount(): Promise<number | null> {
        return readLocalCandidates().length;
    }
    ...
    private async runAnalysis(jobs: Job[], candidates: Candidate[], opts?: { manual?: boolean }) {
        ...
        const localStoreCandidateCount = await this.fetchLocalWorkspaceCandidateCount();
    ...
    private detectAnomalies(current: PipelineSnapshot, previous: PipelineSnapshot | null) {
        ...
        // 3) Talent pool availability: detect an unexpected local candidate-count drop.
        const curPool = current.talentPool.localStoreCandidateCount;
        const prevPool = previous.talentPool.localStoreCandidateCount;
        if (typeof curPool === 'number' && typeof prevPool === 'number' && curPool < prevPool - 1000) {
            this.emitAlert({
                severity: 'error',
                title: 'Talent Pool Size Drop',
                message: `Local candidate pool count dropped from ${prevPool} to ${curPool}. Verify recent imports and browser storage.`,
