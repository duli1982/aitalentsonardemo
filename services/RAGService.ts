...
class RAGService {
    ...
    private async getCandidateRelationships(candidateId: string): Promise<{
        ...
        const candidate = readLocalCandidates().find((item) => item.id === candidateId);
        if (!candidate) return { companies: [], schools: [], topSkills: [] };
        const metadata = candidate.metadata || {};
        const companies = Array.isArray((candidate as any).companies) ? (candidate as any).companies.map(String) : metadata.company ? [String(metadata.company)] : [];
        const schools = Array.isArray((candidate as any).schools) ? (candidate as any).schools.map(String) : [];
        return { companies, schools, topSkills: (candidate.skills || []).slice(0, 5) };
    }
...
Your response:`;
    ...
    /**
     * Check if RAG is available (requires both LocalWorkspace and AI)
     */
    isAvailable(): boolean {
