...
export const useCandidateOperations = ({
   ...
   * - Works for demo candidates and LocalWorkspace candidates.
   * - If a LocalWorkspace candidate is not yet in local state, it is imported into `uploadedCandidates`.
   */
  const handleAddCandidateToPipeline = useCallback((candidate: Candidate, jobId: string, initialStage: PipelineStage = 'new') => {
    ...
    if (!isInInternal && !isInPast && !isInUploaded) {
      // Import LocalWorkspace candidate into local uploadedCandidates for pipeline tracking
