...
export const useJobOperations = ({
    ...
    const handleAddJob = useCallback(async (job: Job, onAutoAnalyze?: (candidates: Candidate[]) => void) => {
        ...
        showToast(
            `Job "${job.title}" added! Found ${strongMatches} strong match${strongMatches !== 1 ? 'es' : ''} (≥70%) and ${goodMatches} good match${goodMatches !== 1 ? 'es' : ''} (50-69%) from ${scoredCandidates.length} candidates.`,
            'success',
            8000
        );
    ...
    const handleUpdateJobStatus = useCallback((jobId: string, newStatus: Job['status']) => {
