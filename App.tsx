...
import TalentCommandCenterPage from './pages/TalentCommandCenterPage';
import RequisitionWorkspacePage from './pages/RequisitionWorkspacePage';
...
import FollowUpsPage from './pages/RecruiterFollowUpsPage';
import EngagementHubPage from './pages/EngagementHubPage';
import RecruitingPortfolioAnalyticsPage from './pages/RecruitingPortfolioAnalyticsPage';
import ManagerDashboardPage from './pages/ManagerDashboardPage';
import MyWorkPage from './pages/MyWorkPage';
import TalentAttractionPage from './pages/TalentAttractionPage';
import CampaignLandingPage from './pages/CampaignLandingPage';
import ScreeningEngagementPage from './pages/ScreeningEngagementPage';
import ExternalScreeningPage from './pages/ExternalScreeningPage';
import CandidateConversationPlatformPage from './pages/CandidateConversationPlatformPage';
import ExternalConversationPage from './pages/ExternalConversationPage';
...
import CareerConfirmationPage from './pages/CareerConfirmationPage';
import CareerApplicationSync from './components/CareerApplicationSync';
import ConnectedShowcaseBootstrap from './components/ConnectedShowcaseBootstrap';
...
const AppContent = () => {
  ...
  const filteredJobs = useMemo(() => {
    return jobs
      .filter(job => job.title.toLowerCase().includes(searchTerm.toLowerCase()) || job.department.toLowerCase().includes(searchTerm.toLowerCase()) || job.location.toLowerCase().includes(searchTerm.toLowerCase()))
  ...
  return (
    <>
      ...
      <CareerApplicationSync />
      <ConnectedShowcaseBootstrap />
      <CandidateJobDrawerProvider openCandidateJobDrawer={openCandidateJobDrawer}>
        ...
        <Routes>
          ...
          >
            <Route index element={<TalentCommandCenterPage />} />
            <Route path="requisitions/:jobId" element={<RequisitionWorkspacePage />} />
            <Route path="candidates/:candidateId" element={<Candidate360Page />} />
            ...
            <Route path="manager-dashboard" element={<ProtectedRoute requiredRoles={['owner', 'admin', 'team_lead', 'sourcing_manager']}><ManagerDashboardPage /></ProtectedRoute>} />
            <Route path="portfolio-analytics" element={<Navigate to="/manager-dashboard" replace />} />
            <Route path="org-twin" element={<ProtectedRoute requiredRoles={['owner', 'admin', 'team_lead', 'sourcing_manager', 'hiring_manager']}><OrgTwinPage /></ProtectedRoute>} />
            ...
            <Route path="hiring-manager" element={<HiringManagerPage />} />
            <Route path="follow-ups" element={<FollowUpsPage />} />
            <Route path="engagement" element={<EngagementHubPage />} />
            <Route path="screening-engagement" element={<ScreeningEngagementPage />} />
            <Route path="conversation-platform" element={<CandidateConversationPlatformPage />} />
            <Route path="attraction" element={<TalentAttractionPage />} />
            <Route path="campaigns/:campaignId/apply" element={<CampaignLandingPage />} />
      ...
      {isSmartSearchModalOpen && (
        <SmartSearchModal
          ...
          onSelectCandidate={(result) => {
            ...
            navigate(`/candidates/${encodeURIComponent(result.id)}`);
            showToast(`Opening ${result.name}'s profile`, 'success');
          }}
        />
      )}
      {isRAGModalOpen && (
...
const App = () => (
  <BrowserRouter>
    <Routes>
      ...
      <Route path="/screen/:organizationId/:token" element={<ExternalScreeningPage />} />
      <Route path="/conversation/:organizationId/:token" element={<ExternalConversationPage />} />
      <Route path="/careers/:organization" element={<CareerSitePage />} />
      <Route path="/careers/:organization/jobs/:jobSlug" element={<CareerJobPage />} />
      <Route path="/careers/:organization/jobs/:jobSlug/apply" element={<CareerApplyPage />} />
      <Route path="/careers/:organization/application/:token" element={<CareerConfirmationPage />} />
      <Route
        path="/*"
        element={
