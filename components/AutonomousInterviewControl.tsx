...
import { readLocalCandidates } from '../services/LocalCandidateStore';
...
const resolveLocalWorkspaceCandidateById = async (id: string): Promise<Candidate | null> => {
    return readLocalCandidates().find((candidate) => candidate.id === id) || null;
};
...
const AutonomousInterviewControl: React.FC<AutonomousInterviewControlProps> = ({ jobs }) => {
    ...
    const startFromScheduled = async (interview: ScheduledInterview) => {
        ...
        if (!candidate) {
            candidate = await resolveLocalWorkspaceCandidateById(String(interview.candidateId)) || undefined;
        }
    ...
    const handleGenerateQuestions = async () => {
        ...
        try {
            const candidate = allCandidates.find((c) => c.id === activeSession.candidateId) || await resolveLocalWorkspaceCandidateById(activeSession.candidateId);
            ...
            <div className="bg-slate-800 rounded-xl border border-slate-700 p-6">
                <div className="flex items-center justify-between mb-6">
                    <div className="flex items-center space-x-3">
                        ...
                        <div>
                            <h3 className="text-lg font-semibold text-white">Simulated Interview Prototype</h3>
                            <p className="text-xs font-semibold text-amber-300">Demo transcript and debrief workflow</p>
