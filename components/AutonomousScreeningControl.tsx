...
const AutonomousScreeningControl: React.FC<AutonomousScreeningControlProps> = ({ jobs }) => {
    ...
    const [candidateSource, setCandidateSource] = useState<'pipeline' | 'localStore' | 'manual'>('pipeline');
    ...
    const {
        ...
        hasMore: localStoreHasMore,
        loadMore: loadMoreLocalWorkspace
    } = useAllLocalWorkspaceCandidates({ enabled: candidateSource === 'localStore', limit: 200 });

    const filteredLocalWorkspaceCandidates = useMemo(() => {
        const normalizedQuery = searchQuery.trim().toLowerCase();
        if (!normalizedQuery) return localStoreCandidates;

        return localStoreCandidates.filter((candidate) => {
            const haystack = [
                candidate.name,
                candidate.email || '',
                ...(candidate.skills || [])
    ...
    }, [localStoreCandidates, searchQuery]);

    const selectedCandidate = useMemo(() => {
        if (candidateSource === 'pipeline') {
            return pipelineCandidatesForJob.find((c) => c.id === selectedCandidateId) || null;
        }
        if (candidateSource === 'localStore') {
            return filteredLocalWorkspaceCandidates.find((c) => c.id === selectedCandidateId) || null;
        }
        return null;
    }, [candidateSource, filteredLocalWorkspaceCandidates, pipelineCandidatesForJob, selectedCandidateId]);
    ...
    const handleQueue = () => {
        ...
        if (candidateSource === 'manual') {
            ...
            const candidate = filteredLocalWorkspaceCandidates.find((c) => c.id === selectedCandidateId);
            if (!candidate) return;
            queueCandidate(candidate);
        }
            ...
            <div className="bg-slate-800 rounded-xl border border-slate-700 p-6">
                <div className="flex items-center justify-between mb-6">
                    <div className="flex items-center space-x-3">
                        ...
                        <div>
                            <h3 className="text-lg font-semibold text-white">Simulated Screening Prototype</h3>
                            <p className="text-xs font-semibold text-amber-300">Uses generated candidate answers; not an external conversation</p>
                    ...
                    <div>
                        ...
                        <div className="grid grid-cols-3 gap-2">
                            ...
                            <button
                                type="button"
                                onClick={() => {
                                    setCandidateSource('localStore');
                                    setSelectedCandidateId('');
                                }}
                                className={`px-3 py-2 rounded-lg border text-sm flex items-center justify-center gap-2 ${
                                    candidateSource === 'localStore'
                                        ? 'bg-slate-700 text-white border-slate-600'
                                        : 'bg-slate-900 text-slate-300 border-slate-700 hover:bg-slate-800'
                                }`}
                            >
                    ...
                    <div className="mt-4 space-y-3">
                        {localStoreError && (
                            <div className="text-sm text-red-300 bg-red-500/10 border border-red-500/20 rounded-lg p-3">
                                Failed to load local candidates: {localStoreError.message}
                            </div>
                        )}
                        <div className="flex items-center justify-between">
                            <div className="text-xs text-slate-500">
                                {localStoreLoading ? 'Loading...' : `${filteredLocalWorkspaceCandidates.length} candidates loaded`}
                            </div>
                            {localStoreHasMore && (
                                <button
                                    type="button"
                        ...
                        <div className="bg-slate-900/50 border border-slate-700 rounded-lg max-h-56 overflow-auto">
                            {filteredLocalWorkspaceCandidates.length === 0 ? (
                                <div className="p-4 text-sm text-slate-400">No local candidates match your search.</div>
                            ) : (
                                <div className="divide-y divide-slate-800">
                                    {filteredLocalWorkspaceCandidates.slice(0, 50).map((candidate) => {
                                        const email = candidate.email || '';
                                        const selected = selectedCandidateId === candidate.id;
                        ...
                        {filteredLocalWorkspaceCandidates.length > 50 && (
