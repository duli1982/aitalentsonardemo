...
import { useAllLocalWorkspaceCandidates } from '../hooks/useAllLocalWorkspaceCandidates';
...
const CandidatesView: React.FC<CandidatesViewProps> = ({ selectedCandidateId, onSelectCandidate, onInitiateAnalysis, onUpdateCandidate, onAddCandidateToPipeline, onUpdateCandidateStage }) => {
  ...
  // Data source toggle: 'demo' or 'localStore'
  const [dataSource, setDataSource] = useState<'demo' | 'localStore'>('demo');
  ...
  // Fetch LocalWorkspace candidates
  const {
    candidates: localStoreCandidates,
    isLoading: isLoadingLocalWorkspace,
    hasMore: hasMoreLocalWorkspace,
    loadMore: loadMoreLocalWorkspace,
    refresh: refreshLocalWorkspace,
    total: localStoreTotal
  } = useAllLocalWorkspaceCandidates({
    enabled: dataSource === 'localStore',
    limit: 100
  });
  ...
  const allCandidates = useMemo(() =>
    dataSource === 'localStore' ? localStoreCandidates : demoCandidates,
    [dataSource, localStoreCandidates, demoCandidates]
  );
  ...
  const booleanSearch = useMemo(() => searchCandidates(searchTerm, allCandidates), [allCandidates, searchTerm]);
  const filteredCandidates = useMemo(() => {
    return booleanSearch.candidates.filter(candidate => {
      ...
      return matchesType && matchesSkills && matchesRole && matchesLocation;
    });
  }, [booleanSearch.candidates, typeFilter, skillFilter, roleFilter, locationFilter]);
    ...
    <div className="flex flex-col md:flex-row h-full gap-6">
      ...
      <div className="w-full md:w-1/3 lg:w-1/4 bg-slate-800 rounded-xl shadow-xl flex flex-col overflow-hidden sticky top-4 h-[calc(100vh-120px)]">
        ...
        <div className="p-4 border-b border-slate-700 space-y-3">
          <div className="flex items-center justify-between gap-2">
            ...
            <div className="flex items-center gap-2">
              <div className="flex items-center bg-slate-700/70 border border-slate-600 rounded-lg overflow-hidden">
                ...
                <button
                  type="button"
                  onClick={() => setDataSource('localStore')}
                  className={`px-2.5 py-1.5 text-[11px] font-semibold flex items-center gap-1.5 transition-colors ${dataSource === 'localStore' ? 'bg-slate-900/50 text-white' : 'text-slate-300 hover:text-white'}`}
          ...
          <div className="flex items-center justify-between text-[11px] text-slate-400">
            <div>
              {dataSource === 'localStore'
                ? (isLoadingLocalWorkspace ? 'Loading local workspace…' : `${localStoreTotal} in local workspace`)
                : `${demoCandidates.length} in Demo`}
            </div>
            {dataSource === 'localStore' && (
              <button
                type="button"
                onClick={refreshLocalWorkspace}
                disabled={isLoadingLocalWorkspace}
                ...
                title="Refresh local workspace"
              >
                <RefreshCw className={`h-3 w-3 ${isLoadingLocalWorkspace ? 'animate-spin' : ''}`} />
                Refresh
              </button>
            )}
          </div>
          ...
          />
          {booleanSearch.error && <p className="rounded-lg border border-red-400/25 bg-red-400/10 p-2 text-xs text-red-200">Boolean search: {booleanSearch.error}</p>}
        ...
        {showInsights && (
          <div className="px-4 py-3 border-b border-slate-700 space-y-3">
            ...
            {dataSource === 'localStore' && !isLoadingLocalWorkspace && localStoreCandidates.length > 0 && (
        ...
        <CandidateList
          ...
          isLoading={dataSource === 'localStore' && isLoadingLocalWorkspace}
        />

        {/* Load More Button for LocalWorkspace */}
        {dataSource === 'localStore' && hasMoreLocalWorkspace && !isLoadingLocalWorkspace && (
          <div className="px-4 pb-3">
            <button
              onClick={loadMoreLocalWorkspace}
