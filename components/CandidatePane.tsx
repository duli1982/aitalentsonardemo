...
const CandidateCard: React.FC<{ candidate: Candidate; job: Job; onInitiateAnalysis: (type: string, candidate: Candidate) => void; onFeedback: (candidateId: string, jobId: string, feedback: 'positive' | 'negative') => void; onViewProfile?: (candidate: Candidate) => void; isLoading: boolean; loadingCandidateId: string | null; }> = ({ candidate, job, onInitiateAnalysis, onFeedback, onViewProfile, isLoading, loadingCandidateId }) => {
    ...
    return (
        <div
            ...
            className="bg-slate-800 shadow-lg rounded-xl p-4 flex flex-col justify-between transition-all duration-300 hover:shadow-sky-500/20 hover:ring-1 hover:ring-slate-700 cursor-pointer"
        >
            <div>
                <div className="flex justify-between items-start mb-3">
                    <div>
                     ...
                     <div className="flex items-center space-x-1">
                        <button onClick={(e) => { e.stopPropagation(); onFeedback(candidate.id, job.id, 'positive'); }} className={`p-1.5 rounded-full transition-colors ${feedback === 'positive' ? 'bg-green-500/30 text-green-300' : 'text-gray-400 hover:text-green-400 hover:bg-slate-700'}`} aria-label="Good match"><ThumbsUp size={16} /></button>
                        <button onClick={(e) => { e.stopPropagation(); onFeedback(candidate.id, job.id, 'negative'); }} className={`p-1.5 rounded-full transition-colors ${feedback === 'negative' ? 'bg-red-500/30 text-red-300' : 'text-gray-400 hover:text-red-400 hover:bg-slate-700'}`} aria-label="Bad match"><ThumbsDown size={16} /></button>
                    </div>
                </div>
                 <div className="mb-3">
...
const CandidatePane: React.FC<CandidatePaneProps> = ({ job, internalCandidates, pastCandidates, uploadedCandidates, onInitiateAnalysis, onFeedback, onBatchAnalysis, onViewProfile, isLoading, loadingCandidateId, isBatchAnalyzing }) => {
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
                {sortedCandidates.length > 0 ? (
                    sortedCandidates.map(candidate => (
                        <CandidateCard key={candidate.id} candidate={candidate} job={job} onInitiateAnalysis={onInitiateAnalysis} onFeedback={onFeedback} onViewProfile={onViewProfile} isLoading={isLoading} loadingCandidateId={loadingCandidateId} />
                    ))
                ) : (
