            ...
            <div className="bg-slate-800 rounded-2xl shadow-2xl w-full max-w-4xl my-8 border border-slate-700">
                        ...
                        <div className="space-y-3">
                            ...
                            {results.map((result) => (
                                <button
                                    type="button"
                                    key={result.id}
                                    className="w-full rounded-lg border border-slate-700 bg-slate-900/50 p-4 text-left transition-all hover:border-purple-500/50 hover:bg-slate-900/70 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-purple-400/70"
                                    onClick={() => onSelectCandidate?.(result)}
                                    aria-label={`Open ${result.name}'s candidate profile`}
                                >
                                        ...
                                        <div className="text-right">
                                            <div className="flex items-center space-x-1 text-purple-400">
                                                ...
                                                <span className="text-sm font-bold">
                                                    {result.hybridScore ?? Math.round(result.similarity * 100)}%
                                                </span>
                                            </div>
                                            <p className="text-xs text-slate-500 mt-1">Hybrid match</p>
                                        </div>
                                    </div>
                                    ...
                                    {result.rankingReasons?.length ? (
                                        <p className="text-xs text-slate-500">{result.rankingReasons.slice(1, 3).join(' • ')}</p>
                                    ) : null}
                ...
                <div className="p-4 border-t border-slate-700 bg-slate-900/50">
                    <p className="text-xs text-slate-500 text-center">
                        Results use deterministic hybrid ranking: semantic relevance plus structured fit
                    </p>
                </div>
            </div>
        </div>
