...
import { useLocalWorkspaceCandidates, type LocalWorkspaceCandidateResult } from '../../hooks/useLocalWorkspaceCandidates';
...
const CandidateJobDrawer: React.FC<CandidateJobDrawerProps> = ({
  ...
  const { candidates: graphMatches, isLoading: graphLoading } = useLocalWorkspaceCandidates(job, {
  ...
  const graphCandidates = useMemo((): Candidate[] => {
    ...
    const fromMatches = (graphMatches || []).map((c: LocalWorkspaceCandidateResult) => ({
      id: String(c.id),
      name: c.name || 'Unknown',
      email: c.email || '',
