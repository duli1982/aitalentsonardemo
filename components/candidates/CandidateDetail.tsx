...
const CandidateDetail: React.FC<CandidateDetailProps> = ({ candidate, jobs, onInitiateAnalysis, onAddToPipeline, onUpdateCandidateStage, onUpdateCandidate }) => {
    ...
    useEffect(() => {
        ...
        let cancelled = false;
        const cacheKey = `engagement_score:recorded:v2:${activeOrganization.organizationId}:${candidate.id}`;
        ...
        const load = async () => {
            ...
            try {
                const messages = await sharedOperationsService.listMessages(activeOrganization.organizationId);
                const score = buildRecordedEngagementScore(candidate, messages);
                if (cancelled) return;
    ...
    }, [activeOrganization.organizationId, candidate?.id]);
    ...
    const loadEngagementScore = async (_mode: 'estimated' | 'ai' = 'estimated') => {
        if (!candidate) return;
        try {
            setEngagementLoading(true);
            setEngagementError(null);
            ...
            setEngagementScore(score);
            try {
                localStorage.setItem(`engagement_score:recorded:v2:${activeOrganization.organizationId}:${candidate.id}`, JSON.stringify({ createdAt: Date.now(), value: score }));
            } catch {
                // ignore cache write errors
            }
        } catch (error) {
    ...
    return (
        <div className="flex flex-col h-full">
            ...
            <CandidateProfileWorkspace
                ...
                scorecardsByJobId={scorecardsByJobId}
                nextAction={nextAction}
                actionInProgress={actionInProgress}
                semanticLoading={semanticJobMatchLoading}
                semanticError={semanticJobMatchError}
                engagementScore={engagementScore}
                engagementLoading={engagementLoading}
                engagementError={engagementError}
                candidateTags={candidateTags}
                scheduledInterviews={scheduledInterviews}
                scheduledInterviewsLoading={scheduledInterviewsLoading}
                interviewSessions={interviewSessions}
                interviewSessionsLoading={interviewSessionsLoading}
                pipelineEvents={pipelineEvents}
                pipelineEventsLoading={pipelineEventsLoading}
                onNextAction={() => void handleNextActionCTA()}
                onAnalyze={(job) => onInitiateAnalysis(candidate, job)}
                onSchedule={(job) => handleOpenModal('schedule', job)}
                onEmail={() => {
                    const jobForEmail = jobs.find(job => candidate.matchScores?.[job.id]) || jobs[0];
                    if (jobForEmail) handleOpenModal('email', jobForEmail);
                }}
                onRefreshEngagement={() => void loadEngagementScore('estimated')}
                onViewScreening={setSelectedScreeningId}
                onViewInterview={setSelectedInterviewSessionId}
                onUpdateStage={onUpdateCandidateStage
                    ? (jobId, stage) => onUpdateCandidateStage(String(candidate.id), jobId, stage)
                    : undefined}
                onUpdateCandidate={onUpdateCandidate ? (updates) => onUpdateCandidate(String(candidate.id), updates) : undefined}
            />
