...
export type CandidateLanguageLevel = 'A1' | 'A2' | 'B1' | 'B2' | 'C1' | 'C2' | 'native' | 'unknown';

export interface CandidateLanguageAssessment {
  language: string;
  level: CandidateLanguageLevel;
  rating?: number;
  assessmentName?: string;
  assessedAt?: string;
  source: 'resume' | 'candidate' | 'assessment' | 'recruiter';
  verified: boolean;
}
...
export interface CandidateConsentRecord {
  status: 'pending' | 'permitted' | 'expired' | 'opted_out';
  capturedAt?: string;
  expiresAt?: string;
  optedOutAt?: string;
  source?: 'candidate' | 'recruiter' | 'import' | 'system';
}

export interface CandidateResumeProvenance {
  parser: 'server-ai-gateway';
...
export interface Candidate {
  ...
  pipelineHolds?: Record<string, { heldAt: string; reason: string; actorType?: 'agent' | 'user' | 'system' }>;
  ...
  languages?: CandidateLanguageAssessment[];
  clientSubmissions?: CandidateClientSubmission[];
  consent?: CandidateConsentRecord;
  resumeProvenance?: CandidateResumeProvenance;
  ...
  // Generic metadata for LocalWorkspace/graph candidates.
  metadata?: Record<string, unknown>;
...
export type EvidenceSnippetSource = 'resume' | 'profile' | 'localStore_document' | 'manual_note' | 'inferred';
