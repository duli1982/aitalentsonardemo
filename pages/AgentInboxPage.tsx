...
const AgentInboxPage: React.FC = () => {
  ...
  const loadItems = useCallback(async () => {
    setItems(proposedActionService.list().filter((item) => item.payload.type !== 'ACTIVATE_RESUME_DRAFT'));
  }, []);
  ...
  const dismiss = (action: ProposedAction) => {
    if (action.payload.type === 'ACTIVATE_RESUME_DRAFT') return;
  ...
  return (
    <div className="mx-auto w-full max-w-7xl space-y-6">
      ...
      <div className="space-y-6">
        <div className="rounded-2xl border border-slate-700 bg-gradient-to-br from-slate-800 via-slate-900 to-indigo-950 p-6 sm:p-8">
          <div className="flex items-center gap-3">
            <div className="p-3 bg-gradient-to-br from-sky-500 to-indigo-600 rounded-xl">
              <Inbox className="h-6 w-6 text-white" />
            </div>
            <div>
              <p className="text-sm font-medium text-sky-300">Human decision queue</p>
              <h1 className="text-3xl font-bold text-white">Review Inbox</h1>
              <p className="mt-1 text-slate-300">
                Review the evidence, edit in context, then approve or dismiss every recommendation.
              </p>
            </div>
          </div>
          <div className="mt-6 grid grid-cols-3 gap-3"><div className="rounded-xl border border-slate-700 bg-slate-950/30 p-3"><p className="text-xs text-slate-400">Awaiting review</p><p className="mt-1 text-2xl font-bold text-sky-200">{counts.proposed}</p></div><div className="rounded-xl border border-slate-700 bg-slate-950/30 p-3"><p className="text-xs text-slate-400">Applied</p><p className="mt-1 text-2xl font-bold text-emerald-200">{counts.applied}</p></div><div className="rounded-xl border border-slate-700 bg-slate-950/30 p-3"><p className="text-xs text-slate-400">Control</p><p className="mt-1 text-2xl font-bold text-violet-200">Human</p></div></div>
        </div>
        ...
        {filtered.length === 0 ? (
          <div className="bg-slate-800/70 rounded-2xl border border-slate-700 p-10 text-center text-slate-400">
            No {statusLabel[status].toLowerCase()} actions.
          </div>
        ) : (
          <div className="space-y-3">
            {filtered.map((action) => (
              <div key={action.id} className="bg-slate-800/70 rounded-2xl border border-slate-700 p-5">
                <div className="flex items-start justify-between gap-4">
                  ...
                  <div className="flex flex-col gap-2 items-end">
                    <div className="flex gap-2">
                      {action.candidateId && (
                        <button
                          type="button"
                          onClick={() => {
                            navigate(`/candidates/${action.candidateId}`);
                          }}
                      ...
                      {action.jobId && (
                        <button
                          type="button"
                          onClick={() => {
                            navigate(`/requisitions/${action.jobId}`);
                          }}
                          className="px-3 py-2 rounded-lg bg-slate-700 hover:bg-slate-600 text-white text-xs font-semibold"
                        >
                          Open role
                    ...
                    {action.status === 'proposed' && (
                      <div className="flex gap-2">
                        {action.payload.type !== 'ACTIVATE_RESUME_DRAFT' ? (
                          <button
                            type="button"
