...
import { Briefcase, Plus, Search, ChevronDown, Sparkles, Brain, Database, LockKeyhole, RotateCcw } from 'lucide-react';
import JobListItem from '../components/JobListItem';
import JobDetails from '../components/JobDetails';
import RequisitionHealthPanel from '../components/RequisitionHealthPanel';
import RequisitionWorkspaceTabs from '../components/jobs/RequisitionWorkspaceTabs';
...
const JobsPage: React.FC<JobsPageProps> = ({
    ...
    useEffect(() => {
        ...
        const matchingJob = jobs.find((job) => job.status === 'open' && job.location.toLowerCase().includes(location.toLowerCase()));
        if (matchingJob) setSelectedJobId(matchingJob.id);
    }, [jobs, searchParams, setSearchTerm, setSelectedJobId, setStatusFilter]);
    ...
    return (
        <div className="flex flex-col md:flex-row gap-6 h-[calc(100vh-140px)]">
            ...
            <div className="w-full md:w-2/3 lg:w-3/4 flex flex-col space-y-6 h-full">
                {selectedJob ? (
                    <>
                        <JobDetails job={selectedJob} readOnly={selectedJob.status === 'closed'} onAnalyze={() => handleInitiateAnalysis('JOB_SUMMARY', selectedJob)} isAnalyzing={isLoading && analysisState.type === 'JOB_SUMMARY'} onUpdateStatus={handleUpdateJobStatus} />
                        {selectedJob.status === 'closed' && <div className="flex flex-col gap-3 rounded-xl border border-amber-400/25 bg-amber-400/10 px-4 py-3 sm:flex-row sm:items-center sm:justify-between"><div className="flex items-start gap-3"><LockKeyhole className="mt-0.5 h-5 w-5 shrink-0 text-amber-300" /><div><p className="font-semibold text-amber-100">This requisition is closed and read-only</p><p className="mt-0.5 text-xs text-amber-200/70">Candidate history remains available, but sourcing, shortlisting, analysis, and pipeline changes are disabled.</p></div></div><button type="button" onClick={() => handleUpdateJobStatus(selectedJob.id, 'open')} className="inline-flex shrink-0 items-center justify-center gap-2 rounded-lg border border-amber-300/30 bg-slate-900/40 px-3 py-2 text-sm font-bold text-amber-100 hover:bg-slate-800"><RotateCcw className="h-4 w-4" />Reopen requisition</button></div>}
                        <RequisitionHealthPanel job={selectedJob} compact readOnly={selectedJob.status === 'closed'} />
                        <RequisitionWorkspaceTabs job={selectedJob} candidates={allCandidates} readOnly={selectedJob.status === 'closed'} onViewProfile={handleViewProfile} onAddToPipeline={handleAddCandidateToPipeline} />
                    </>
                ) : jobs.length === 0 ? (
