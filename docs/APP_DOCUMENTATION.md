...
### Backend Services
- **Local workspace PostgreSQL** - Vector database + relational database
- **Google Gemini AI (Flash 2.5)** - AI analysis and generation
- **LocalStorage** - Client-side caching and persistence

### Key Libraries
- **@local workspace/local workspace-js** - Local workspace client
- **@google/genai** - Google Gemini API client
- **EventBus** - Custom event system for inter-component communication

### Data Storage
- **Candidate Documents** - Stored in Local workspace `candidate_documents` table with vector embeddings
...
│         │                    │                    │                │
│    ┌────▼─────┐      ┌───────▼────────┐   ┌──────▼──────┐        │
│    │ Gemini   │      │   Local workspace     │   │  EventBus   │        │
...
- Vector similarity search in Local workspace
- Embedding generation for queries
- Threshold-based filtering
- Result ranking by similarity

#### 5. **Local workspaceClient**
...
- **Vector Embeddings**: Each candidate resume is converted to embeddings and stored in Local workspace
...
- **Graph Enrichment**: Candidates from Local workspace are enriched with company/school data
...
- **Local workspace Pool**: 10,000+ candidates with vector search
...
4. Agent builds semantic query from job requirements
   ↓
5. SemanticSearchService searches Local workspace vector database
   ↓
...
2. Local workspace performs cosine similarity search against all candidate embeddings
...
T=10min: Agent scans (second run)
  ...
  ↓
T=15min: New candidate Emma Davis uploaded to Local workspace
  ↓
...
private async runAnalysis(jobs: any[], candidates: any[]) {
  ...
  // 2c. Fetch Local workspace candidate count
  const local workspaceCandidateCount = await this.fetchLocal workspaceCandidateCount();
...
```json
{
  ...
  "talentPool": {
    "local workspaceCandidateCount": 10247
  }
}
```

#### Step 4: Local workspace Candidate Count
```typescript
private async fetchLocal workspaceCandidateCount(): Promise<number | null> {
  try {
    const result = await local workspace
...
```typescript
// Local workspace candidate count dropped significantly
const curPool = current.talentPool.local workspaceCandidateCount;
const prevPool = previous.talentPool.local workspaceCandidateCount;
...
if (
  ...
  this.emitAlert({
    ...
    message: `Local workspace candidate pool count dropped from ${prevPool} to ${curPool}. Verify ingestion jobs and data retention policies.`,
...
Local workspace candidate pool count dropped from 10,247 to 8,912. Verify ingestion jobs and data retention policies.
```
...
- Matched candidates for selected job (from Local workspace or demo data)
...
- **Hybrid Data Source Toggle**:
  ...
  - Local workspace Pool (10,000+ candidates with Knowledge Graph)
- Enhanced candidate list with company/school badges
- Best match score display
- Skill highlighting
- Experience & location icons
- "Load More" pagination for Local workspace
- Refresh button
- Knowledge Graph info banner

**Data Sources**:
```typescript
const [dataSource, setDataSource] = useState<'demo' | 'local workspace'>('demo');

const allCandidates = dataSource === 'local workspace'
  ? local workspaceCandidates
  : demoCandidates;
```
...
- Agent health checks
- Local workspace connection status
- API rate limits
...
# Local workspace Configuration (required for vector search and Knowledge Graph)
VITE_LOCAL_WORKSPACE_URL=https://your-project.local workspace.co
VITE_LOCAL_WORKSPACE_ANON_KEY=your_local workspace_anon_key_here
```

### Optional Features
- **Without Gemini API Key**: App works with demo data, but AI analysis features are disabled
- **Without Local workspace**: App works with 66 demo candidates only (no vector search, no Knowledge Graph)
...
# VITE_GEMINI_API_KEY=...
# VITE_LOCAL_WORKSPACE_URL=...
# VITE_LOCAL_WORKSPACE_ANON_KEY=...
...
### 12.1 Local workspace PostgreSQL
...
#### 2. Local workspace Connection Fails
**Symptom**: "Failed to fetch candidates" or empty results

**Solution**:
- Verify `VITE_LOCAL_WORKSPACE_URL` and `VITE_LOCAL_WORKSPACE_ANON_KEY` in `.env.local`
- Check Local workspace project is active
- Verify Row Level Security (RLS) policies allow anonymous reads
- Test connection: `await local workspace.from('candidate_documents').select('id').limit(1)`
...
1. **Vector Search Caching**
   - 5-minute TTL cache for Local workspace queries
...
| **RLS** | Row Level Security (Local workspace access control) |
...
### Documentation
- **Local workspace Docs**: https://local workspace.com/docs
