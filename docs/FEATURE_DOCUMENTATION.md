...
- **Local workspace Vector Database** - Semantic search with pgvector
...
```
Local workspace PostgreSQL + pgvector Extension
...
Step 2: For each candidate:
  ...
  └── Insert into Local workspace candidate_documents table
...
- Requires server env vars: `LOCAL_WORKSPACE_SERVICE_ROLE_KEY` and `GEMINI_API_KEY` (do not expose in client).
...
```
1. Set up Local workspace (add credentials to .env)
2. Run SQL setup from LOCAL_WORKSPACE_VECTOR_GUIDE.md
3. Go to /ingest
...
```
1. Go to Local workspace dashboard
...
1. **Run SQL Setup** in Local workspace SQL Editor:
   ```sql
...
- Insert into Local workspace vector database
...
- **Cause**: Local workspace connection or API key issues
...
1. **Run `sql/KNOWLEDGE_GRAPH_SETUP.sql`** in Local workspace SQL Editor
...
2. **Run `sql/GRAPH_MIGRATION_SETUP.sql`** in Local workspace SQL Editor
...
- Database performance (Local workspace region, plan tier)
- Network latency
...
- **Solution**: Run the SQL script in Local workspace SQL Editor first
...
Step 1: Gemini AI converts to vector [0.2, 0.4, 0.1, ...]
     ↓
Step 2: Local workspace searches candidate vectors
     ↓
...
**Requirements:**
- ✅ Local workspace configured with vector database
...
- ✅ Local workspace configured with `sql/KNOWLEDGE_GRAPH_SETUP.sql` executed
...
2. **Local workspace account** (free tier works)
3. **Google Cloud account** for Gemini AI
4. **Git** for version control

### Environment Setup

1. **Create Local workspace Project**
   ```bash
   1. Go to https://local workspace.com
   2. Click "New Project"
...
2. **Enable Vector Extension**
   ```sql
   -- In Local workspace SQL Editor, run:
   CREATE EXTENSION IF NOT EXISTS vector;
   ```

3. **Create Candidates System-of-Record + Active Documents (Recommended)**
   In Local workspace SQL Editor, run the repo script:
   ```sql
...
4. **Setup Knowledge Graph (NEW)**
   ```sql
   -- In Local workspace SQL Editor, run sql/KNOWLEDGE_GRAPH_SETUP.sql
   -- This creates:
...
5. **Setup Bulk Ingestion Progress Tracking**
   ```sql
   -- In Local workspace SQL Editor, run sql/BULK_INGESTION_SETUP.sql
...
6. **Setup Knowledge Graph Migration Tracking (IMPORTANT for Existing Data)**
   ```sql
   -- In Local workspace SQL Editor, run sql/GRAPH_MIGRATION_SETUP.sql
...
7. **Configure Environment Variables**
   ...
   VITE_LOCAL_WORKSPACE_ANON_KEY=your_anon_key_here
...
│            External Services                    │
│  ┌──────────────┐  ┌──────────────────────────┐│
│  │ Google       │  │ Local workspace PostgreSQL      ││
│  │ Gemini AI    │  │ + pgvector               ││
...
User Query
  → SemanticSearchService.search()
    → AIService.embedText() (Google Gemini)
      → Get vector [0.1, 0.5, ...]
    → local workspace.rpc('match_candidates')
...
**Symptoms:**
```
✗ Migration failed: local workspaceUrl is required
```

**Solution:**
1. Check `.env` file has Local workspace credentials
2. Restart dev server: `Ctrl+C` then `npm run dev`
3. Verify Local workspace project is active
...
2. Verify Local workspace has data:
   ```sql
   SELECT COUNT(*) FROM candidate_documents;
   -- Should return > 0
   ```
...
- Check Local workspace status: https://status.local workspace.com
...
- [LOCAL_WORKSPACE_VECTOR_GUIDE.md](./LOCAL_WORKSPACE_VECTOR_GUIDE.md) - Vector DB setup
- [CLAUDE.md](./CLAUDE.md) - Development guidelines

**External Docs:**
- [Local workspace Vector Guide](https://local workspace.com/docs/guides/ai/vector-columns)
...
4. Check Local workspace dashboard for data
...
│   └── local workspaceClient.ts               # Database client
│
├── components/
│   ├── modals/
